...
def iterative_deepening_depth_first_search(maze_map, start_pos, goal_pos):
    ...
    start = (start_pos[0][0],start_pos[0][1],0)
    goal = goal_pos
    queueSize = 1
    queue = deque([start])
    queueNextLevel = deque()
    queueNextLevelSize = 0

    # the queueNextLevel saves the nodes of the next Iteration Level
    ...
    try:
        while goal_pos > 0:
            ...
            print("queueSize: ")
            print(queueSize)
            print("queueSizeNextLevel: ")
            print(queueNextLevelSize)
            ...
            if queueSize > 0:
                tuple = queue.pop()
                queueSize -= 1
            elif queueNextLevelSize > 0:
                while queueNextLevel:
                    queue.appendleft(queueNextLevel.pop())
                    queueSize += 1
                    queueNextLevelSize -=1
                    iterationSize += 3
            else:
                return
            ...
            if x == 0:
                ...
                s += "\u2575"
                if ufree== 0:
                    if tuple[2]+1 > iterationSize:
                        queueNextLevel.append(((tuple[0]-1),(tuple[1]),tuple[2]+1)) #+1
                        queueNextLevelSize += 1
                        expMatrix[(tuple[0]-1)][(tuple[1])] = 1
                    else:
                        queue.append(((tuple[0]-1),(tuple[1]),tuple[2]+1)) #+1
                        queueSize += 1
                        expMatrix[(tuple[0]-1)][(tuple[1])] = 1
            elif x == 2:
                s += "\u2576"
                if rfree == 0:
                    if tuple[2]+1 > iterationSize:
                        queueNextLevel.append(((tuple[0]),(tuple[1]+1),tuple[2]+1)) #+2
                        queueNextLevelSize += 1
                        expMatrix[(tuple[0])][(tuple[1]+1)] = 1
                    else:
                        queue.append(((tuple[0]),(tuple[1]+1),tuple[2]+1)) #+2
                        queueSize += 1
                        expMatrix[(tuple[0])][(tuple[1]+1)] = 1
            elif x == 3:
                s += "\u2514"
                ...
                if bfree == 0:
                    if tuple[2]+1 > iterationSize:
                        queueNextLevel.append(((tuple[0]+1),(tuple[1]),tuple[2])) #+4
                        queueNextLevelSize += 1
                        expMatrix[(tuple[0]+1)][(tuple[1])] = 1
                    else:
                        queue.append(((tuple[0]+1),(tuple[1]),tuple[2])) #+4
                        queueSize += 1
                        expMatrix[(tuple[0]+1)][(tuple[1])] = 1
            elif x == 5:
                s += "\u2502"
                ...
                if lfree == 0:
                    if tuple[2]+1 > iterationSize:
                        queueNextLevel.append(((tuple[0]),(tuple[1]-1),tuple[2])) #+8
                        queueNextLevel += 1
                        expMatrix[(tuple[0])][(tuple[1]-1)] = 1
                    else:
                        queue.append(((tuple[0]),(tuple[1]-1),tuple[2])) #+8
                        queueSize += 1
                        expMatrix[(tuple[0])][(tuple[1]-1)] = 1
            elif x == 9:
                s += "\u2518"
